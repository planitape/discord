...
export function start() {
    if (window.__EU_VC_SINGLETON__) return;
    window.__EU_VC_SINGLETON__ = true;

    // --- VC storage adapter (generates its own config file) ---
    const Storage = {
        load() {
            try {
                if (settings.store.config) return settings.store.config;
                const j = localStorage.getItem("EU_cfg");
                return j ? JSON.parse(j) : null;
            } catch { return null; }
        },
        save(cfg) {
            try {
                if (settings.store.config) return settings.store.config = cfg;
                localStorage.setItem("EU_cfg", JSON.stringify(cfg));
            } catch { }
        }
    };
    ...
    const boot = (() => {
        ...
        const EXTRA_DELETE_ON_SAVE = false;
        const CAPTURE_PASTE_ALWAYS = true;
        const SUPPORT_URL = "/invite/SADyYHbWn5";
        ...
        const rgba = (hex, a = 1) => {
            ...
            return `rgba(${parseInt(m[1] || "ff", 16)},${parseInt(m[2] || "00", 16)},${parseInt(m[3] || "00", 16)},${a})`;
        };
        const fiber = n => { for (const k in n) if (k.startsWith("__reactFiber$")) return n[k]; };

        // find File/Blob in React props
        const deepFindFile = (x, s = new WeakSet()) => {
        ...
        // clone SVGs from native Discord buttons when available
        const cloneByAriaIncludes = list => {
            for (const t of list) {
                const b = qa(`button[aria-label*="${t}"],[aria-label*="${t}"]`).find(x => x.offsetParent !== null);
        ...
        const ICON = {
            ...
            pencil: "<svg viewBox=\"0 0 24 24\"><path fill=\"currentColor\" d=\"M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zm15.71-9.04a1.003 1.003 0 0 0 0-1.42l-2.5-2.5a1.003 1.003 0 0 0-1.42 0l-1.83 1.83 3.75 3.75 1.99-1.66z\"/></svg>"
        };
        ...
        const firstRemoveAttachmentBtn = () => {
            const r = getComposer();
            const sel = [
            ...
            return qa(sel, r).find(b => b.offsetParent !== null) || null;
        };
        const composerHasAttachments = () => !!firstRemoveAttachmentBtn();
        const sendNow = () => {
            const root = getComposer();
            const btn = qa([
                'button[aria-label="Send Message"]', 'button[aria-label="Send"]', 'button[aria-label*="Send"]',
                'button[type="submit"]', '[data-testid="send-button"]', '[class*="buttons"] button[aria-label*="Send"]'
            ...
            if (box) { box.focus(); const k = t => new KeyboardEvent(t, { key: "Enter", code: "Enter", which: 13, keyCode: 13, bubbles: true, cancelable: true, composed: true });["keydown", "keypress", "keyup"].forEach(t => box.dispatchEvent(k(t))); return true; }
            const form = root; if (form) { form.dispatchEvent(new Event("submit", { bubbles: true, cancelable: true })); return true; }
            return false;
        };
        const purgeComposer = () => {
            let rounds = 0; (function tick() {
                let hit = false;
        ...
        const biggestNonAvatarImg = root => {
            const imgs = qa("img", root).filter(i => {
                const cls = (i.className || "") + " " + (i.parentElement?.className || "");
                const aria = (i.getAttribute("aria-label") || "").toLowerCase();
                return !/(^| )avatar|pfp|mask|user|guild/i.test(cls + aria);
            });
            let best = null, area = 0; for (const im of imgs) { const w = im.naturalWidth || im.width || 0, h = im.naturalHeight || im.height || 0; if (w * h > area) { area = w * h; best = im; } }
            return best;
        };

        // minimal stylesheet (same UI as native web script)
        if (!q("#eu-style")) {
            const st = document.createElement("style"); st.id = "eu-style"; st.textContent = `
...
#eu-support svg{width:18px;height:18px;fill:currentColor}
...
.eu-btn svg{width:28px;height:28px;color:currentColor;fill:currentColor}
.eu-btn.icon{width:52px;height:44px}
.eu-btn.icon svg{width:36px;height:36px}
#eu-settings{color:var(--interactive-normal)}
#eu-settings svg{fill:currentColor!important;color:currentColor!important}
#eu-zoom-in svg,#eu-zoom-out svg{width:38px;height:38px}
#eu-undo svg,#eu-redo svg{width:36px;height:36px}
#eu-download svg{width:40px;height:40px}
...
`; document.head.appendChild(st);
        ...
        // editor overlay (same logic/UI as native web script)
        function openEditorWithBlob(blob) {
            if (document.getElementById("eu-overlay")) return;
  ...
  <div id="eu-panel" role="dialog" aria-modal="true" aria-label="EditUploadsV1">
   ...
   <div id="eu-stage-wrap"><div id="eu-stage"><canvas id="eu-cc"></canvas><canvas id="eu-cv"></canvas><canvas id="eu-overlay-cv"></canvas><div id="eu-borderglow" class="eu-burst"></div></div></div>
            ...
            // trap focus/keys inside overlay
            ...
            const HSVtoHEX = (h, s, v) => { const f = (n, k = (n + h / 60) % 6) => v - v * s * Math.max(Math.min(k, 4 - k, 1), 0); const r = Math.round(f(5) * 255), g = Math.round(f(3) * 255), b = Math.round(f(1) * 255); return "#" + [r, g, b].map(x => x.toString(16).padStart(2, "0")).join(""); };
            ...
            // settings picker for static RGB
            function initRgbPicker(hex0) {
                const H2 = HEXtoHSV(hex0); chip2.style.background = hex0;
            ...
            function applyGlowAnimation() {
                if (st.rgbMode !== "dynamic") { borderGlow.style.animation = ""; return; }
            ...
            function setGlowBase() {
                if (st.rgbMode === "off") { borderGlow.style.display = "none"; return; }
            ...
            function setTool(name) {
                st.tool = name; qa(".eu-tool", ov).forEach(b => b.classList.toggle("active", b.dataset.tool === name));
            ...
            // attachment trash visibility
            const refreshTrashVisibility = () => trashBtn.style.display = composerHasAttachments() ? "" : "none";
            const trashObs = new MutationObserver(refreshTrashVisibility); trashObs.observe(getComposer(), { subtree: true, childList: true, attributes: true }); refreshTrashVisibility();
            ...
            function initFromImage(img) {
                cv.width = img.width; cv.height = img.height; applyCanvasDimsToDOM(); cx.clearRect(0, 0, cv.width, cv.height); cx.drawImage(img, 0, 0);
            ...
            const wheelHandler = e => { if (!e.ctrlKey) return; e.preventDefault(); setZoom(st.zoom * (e.deltaY > 0 ? .9 : 1.1)); };
            stageWrap.addEventListener("wheel", wheelHandler, { passive: false });
            ...
            const mouseMove = e => { if (!panning) return; st.panX = sp.x + (e.clientX - ps.x); st.panY = sp.y + (e.clientY - ps.y); updateTransform(); };
            const mouseUp = () => { panning = false; };
            ...
            // drawing
            stage.onmousemove = e => {
                const p = pos(e); lastCursor = p; if (st.tool === "text") { st.ghost.on = true; st.ghost.x = p.x; st.ghost.y = p.y; }
            ...
            stage.onmousedown = e => {
                ...
                if (st.tool === "text") {
                    ...
                    if (val) { pushUndo(); cx.save(); if (st.textOutline) { cx.strokeStyle = "#000"; cx.lineWidth = 3; cx.font = textFont(); cx.textBaseline = "middle"; cx.strokeText(val, p.x, p.y); } cx.fillStyle = rgba(st.color, st.strength); cx.font = textFont(); cx.textAlign = "left"; cx.textBaseline = "middle"; cx.fillText(val, p.x, p.y); cx.restore(); st.base.getContext("2d").drawImage(cv, 0, 0); st.sharpBase.getContext("2d").drawImage(cv, 0, 0); updateSizeFromCanvas(); burst(); } return;
                }
            ...
            const mouseUpDraw = async e => {
                if (st.placing) { st.placing.drag = false; renderOverlay(); return; }
                if (!st.start) return;
                const r = stageWrap.getBoundingClientRect(), p = { x: (e.clientX - r.left - st.panX) / st.zoom, y: (e.clientY - r.top - st.panY) / st.zoom };
            ...
            addEventListener("mouseup", mouseUpDraw);

            // HDR
            const toneMapSDR = () => { const w = cv.width, h = cv.height, id = cx.getImageData(0, 0, w, h), a = id.data; for (let i = 0; i < a.length; i += 4) { let r = a[i] / 255, g = a[i + 1] / 255, b = a[i + 2] / 255; r /= (1 + r); g /= (1 + g); b /= (1 + b); r = Math.pow(r, 1 / 1.05); g = Math.pow(g, 1 / 1.05); b = Math.pow(b, 1 / 1.05); a[i] = (r * 255) | 0; a[i + 1] = (g * 255) | 0; a[i + 2] = (b * 255) | 0; } cx.putImageData(id, 0, 0); };
            ...
            // settings pop + persistence via VC Data API
            ...
            function applyRGB(mode, color) { st.rgbMode = mode; if (mode === "off") { borderGlow.style.display = "none"; rgbPickerWrap.style.display = "none"; } else if (mode === "static") { st.rgbStatic = color || rgbHEX.value || st.rgbStatic; rgbPickerWrap.style.display = "block"; initRgbPicker(st.rgbStatic); setGlowBase(); } else { rgbPickerWrap.style.display = "none"; setGlowBase(); } }
            ...
            const doSave = async ({ send = false } = {}) => { try { if (send) { await doSend(); return; } if (EXTRA_DELETE_ON_SAVE) purgeComposer(); await saveToQueue(); await updateSizeFromCanvas(); burst(); } catch { alert("Couldn't complete action"); } };
            ...
            // load/save config (BD file-backed)
            function loadCfg() {
                try {
                    const c = Storage.load(); if (!c) return;
                    if (c.rgbMode) st.rgbMode = c.rgbMode;
                    if (c.rgbStatic) st.rgbStatic = c.rgbStatic;
                    if (typeof c.rgbSpeed === "number") st.rgbSpeed = c.rgbSpeed;
                    if (c.hdrMode) st.hdrMode = c.hdrMode;
                    if (c.inject) st.inject = { ...st.inject, ...c.inject };

                    swAttach.checked = !!st.inject.attachments;
                    swViewer.checked = !!st.inject.viewer;
                    swMisc.checked = !!st.inject.misc;
                    speedEl.value = st.rgbSpeed; setFill(speedEl);

                    setGlowBase(); applyHDRMode(); applyGlowAnimation();
                    window.__EU_INJECT_STATE__ = { ...st.inject };
                    sweepEnhance();
                } catch { }
            }
            function saveCfg() {
                try {
                    const cfg = { rgbMode: st.rgbMode, rgbStatic: st.rgbStatic, hdrMode: st.hdrMode, inject: st.inject, rgbSpeed: st.rgbSpeed };
                    Storage.save(cfg);
                    window.__EU_INJECT_STATE__ = { ...st.inject };
                    st.dirty = false; refreshSaveBtn();
                } catch { }
            }
            loadCfg();
            saveSettingsBtn.onclick = () => { saveCfg(); };
            const outsideSettingsClick = e => { if (settingsPop.style.display === "block" && !e.target.closest?.("#eu-settings-pop,#eu-settings")) { settingsPop.style.display = "none"; } };
            addEventListener("click", outsideSettingsClick);

            // paste inside editor (image placement)
            const onPaste = e => {
                const items = e.clipboardData?.items || []; for (const it of items) {
                    if (it.type && it.type.startsWith("image/")) { e.preventDefault(); const blob = it.getAsFile(), u = URL.createObjectURL(blob), img = new Image(); img.onload = () => { enterPlace(img); URL.revokeObjectURL(u); }; img.src = u; return; }
                }
            };
            ov.addEventListener("paste", onPaste, { capture: true });

            function closeAndCleanup() {
                ov.removeEventListener("paste", onPaste, true);
                removeEventListener("click", outsideSettingsClick, false);
                removeEventListener("mousemove", mouseMove);
                removeEventListener("mouseup", mouseUp);
                removeEventListener("mouseup", mouseUpDraw);
                stageWrap.removeEventListener("wheel", wheelHandler, { passive: false });
                trashObs.disconnect();
                ov.remove();
            }
        }

        // inline edit button + tooltip
        ...
        function attachNativeTip(el) {
            ...
            const show = () => {
                if (tip) return; tip = document.createElement("div"); tip.className = "eu-tip"; tip.textContent = el.dataset.euTip || ""; document.body.appendChild(tip);
        ...
        // read file from tile via React fiber or fallback to fetch
        ...
        // injection toggles (respect saved config even before opening editor)
        let injectState = { attachments: true, viewer: true, misc: false };
        (function primeInjectFromConfig() {
            const c = Storage.load();
            if (c?.inject) injectState = { ...injectState, ...c.inject };
            window.__EU_INJECT_STATE__ = { ...injectState };
        })();
        ...
        // BD lifecycle-owned observers/intervals
        ...
        const sweepInterval = setInterval(sweepEnhance, 3500);
        setTimeout(sweepEnhance, 200); setTimeout(sweepEnhance, 800);

        // global paste → open editor
        const globalPasteHandler = e => {
            if (!CAPTURE_PASTE_ALWAYS || q("#eu-overlay")) return;
            const items = e.clipboardData?.items || [];
            for (const it of items) {
                if (it.type && it.type.startsWith("image/")) { e.preventDefault(); openEditorWithBlob(it.getAsFile()); return; }
            }
        };
        document.addEventListener("paste", globalPasteHandler, { capture: true });

        // public toggle helper
        ...
        return function cleanup() {
            try {
                ...
                const overlay = document.getElementById("eu-overlay"); overlay?.remove();
                const style = document.getElementById("eu-style"); style?.remove();
                qa(`button[aria-label="${BTN_ARIA}"]`).forEach(b => b.remove());
                qa(".eu-tip").forEach(t => t.remove());
                delete window.__EU_SET_INJECT__;
                delete window.__EU_INJECT_STATE__;
            } catch { }
        };
    })();
...
let __eu_cleanup: (() => void) | null = null;

export function stop() {
    try { __eu_cleanup && __eu_cleanup(); } catch { }
    __eu_cleanup = null;
    delete window.__EU_VC_SINGLETON__;
}
