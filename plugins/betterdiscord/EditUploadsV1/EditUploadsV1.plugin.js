 ...
 * @version 1.0.1
 * @author ape
 * @description Edit Uploads before sending. Ported to BetterDiscord (discord native web console script)
 * @source https://raw.githubusercontent.com/planitape/discord/refs/heads/main/scripts/EditUploadsV1/EditUploadsV1.js
 * @updateUrl none
 */

module.exports = class EditUploadsV1 {
  start() {
    if (window.__EU_BD_SINGLETON__) return;
    window.__EU_BD_SINGLETON__ = true;

    // --- BD storage adapter (generates its own config file) ---
    const PLUGIN = "EditUploadsV1";
    const KEY = "config";
    const Storage = {
      load() {
        try {
          if (window.BdApi?.Data) return BdApi.Data.load(PLUGIN, KEY) || null;
      ...
      save(cfg) {
        try {
          if (window.BdApi?.Data) return BdApi.Data.save(PLUGIN, KEY, cfg);
  ...
  <div id="eu-panel" role="dialog" aria-modal="true" aria-label="EditUploadsV1">
   ...
   <div id="eu-stage-wrap"><div id="eu-stage"><canvas id="eu-cc"></canvas><canvas id="eu-cv"></canvas><canvas id="eu-overlay-cv"></canvas><div id="eu-borderglow" class="eu-burst"></div></div></div>
        ...
        function initRgbPicker(hex0){
          let H2=HEXtoHSV(hex0); chip2.style.background=hex0;
        ...
        // settings pop + persistence via BD Data API
      ...
      function attachNativeTip(el){
        ...
        const show=()=>{ if(tip) return; tip=document.createElement('div');tip.className='eu-tip';tip.textContent=el.dataset.euTip||''; document.body.appendChild(tip);
    ...
    this.__eu_cleanup = boot;
  }

  stop() {
    try { this.__eu_cleanup && this.__eu_cleanup(); } catch {}
    delete this.__eu_cleanup;
    delete window.__EU_BD_SINGLETON__;
  }
};
